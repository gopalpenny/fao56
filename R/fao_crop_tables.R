...
get_crop_tables <- function() {
  ...
  crop_stages1 <- crop_stages_all[[1]] %>% tibble::as_tibble() %>% dplyr::select(-1) %>% setNames(dplyr::slice(.,1)) %>% # remove first "Crop" column, set header from 1st row
    ...
    dplyr::select(Crop_group,Crop_name,Crop_subcat,`Init. (Lini)`,`Dev. (Ldev)`,`Mid (Lmid)`,`Late (Llate)`,Total,`Plant Date`,Region) #%>%
  ...
  crop_coeff1 <- crop_coeff_all[[1]] %>% #select(-1) %>%
    setNames(gsub("^$","Kcini",dplyr::slice(.,1)))
